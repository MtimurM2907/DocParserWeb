...
Это **не полноценная СЭД** и **не замена КриптоПро в браузере**: УКЭП регистрируется загрузкой отсоединённой подписи и проверкой CMS на сервере (если сертификат есть в хранилище ОС), без плагина CryptoPro.
...
| **Согласование** | Цепочка согласующих, задачи, утверждение / возврат |
| **Подпись** | Внутренняя ЭП (SHA-256), регистрация УКЭП/CMS, PDF с листом подписей |
| **Обмен** | Шаринг, список доступов, отзыв |
| **Экспорт** | DOCX, PDF, подписанный PDF, скачивание оригинала |
| **Версии** | История, восстановление, сравнение двух версий (diff) |
| **Уведомления** | In-app + email, SignalR push в браузер |
| **Блокировка редактирования** | Совместное редактирование: lease + SignalR |
| **Безопасность загрузки** | Размер, расширение, magic bytes PDF/DOCX |
| **LDAP** | Опциональный вход через Active Directory (bind) |
| **Администрирование** | Пользователи, подразделения, audit |
| **Enterprise** | Пакетная загрузка, audit log, чеклисты, сущности, webhook |
...
**Внешняя подпись (`external`):** загрузка `.sig` / `.p7s`, субъект и отпечаток сертификата; при наличии сертификата в хранилище Windows выполняется проверка CMS (`SignedCms`). Экспорт `GET /api/pdf/{id}/export?format=signed-pdf` добавляет лист с данными подписей.

**Realtime:** хаб SignalR `/hubs/document` — события `lockChanged` (блокировка текста) и `notification` (колокольчик в шапке). JWT передаётся query-параметром `access_token`.
...
| POST | `documents/{id}/sign` | Внутренняя подпись |
| POST | `documents/{id}/sign/external` | УКЭП / CMS (multipart) |
| GET | `documents/{id}/signatures` | Список подписей |
| GET/POST/DELETE | `documents/{id}/edit-lock` | Блокировка редактирования |
| GET | `documents/{id}/versions/diff` | Diff двух версий |
| GET/POST | `notifications` | Уведомления |
| CRUD | `departments` | Подразделения (admin) |
...
| `Ldap` | `Enabled`, host, `BindDnTemplate` — вход до проверки пароля в БД |
| `FileScan` | Лимит размера, расширения, проверка сигнатуры файла |
...
- **УКЭП** — нет интеграции с CryptoPro в браузере; только загрузка CMS и серверная проверка при наличии сертификата.
- **LDAP** — простой bind; без синхронизации групп AD → ролей.
- **Мобильная вёрстка** — не оптимизирована (десктопный UI).
- **Гостевой режим** — отключён; документы только для авторизованных пользователей.

Возможные улучшения: полноценный CryptoPro, SSO/SAML, шаблоны маршрутов, полнотекстовый поиск с индексом, антивирусная проверка вложений.
