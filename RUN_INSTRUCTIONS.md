...
#### Настройки GigaChat (нейросеть)

Без ключей приложение работает, но **AI-описание** и **переписывание** недоступны; **орфография** идёт через локальный Hunspell.

1. Зарегистрируйтесь на [developers.sber.ru](https://developers.sber.ru/) и создайте проект **GigaChat API**.
2. Скопируйте в папке `DocParseLab.Server`:
   ```powershell
   copy gigachat.secrets.json.example gigachat.secrets.json
   ```
3. Откройте `gigachat.secrets.json` и вставьте **Client ID** и **Client Secret** (Base64 вручную собирать не нужно):
   ```json
   {
     "GigaChat": {
   ...
   ```
4. Перезапустите сервер. В консоли должно появиться: `GigaChat: учётные данные загружены`.

Альтернатива — переменные окружения: `GigaChat__ClientId`, `GigaChat__ClientSecret`, `GigaChat__Scope`.

Файл `gigachat.secrets.json` в `.gitignore` — не коммитьте его в git.
...
## Тестовые учётные записи (демо-данные)

Остановите сервер, затем выполните (очистит все документы и пользователей, создаст демо-набор):
...
Подразделения: **Администрация**, **Бухгалтерия**, **Юридический отдел**, **Общий отдел**.

Профиль пользователя (ФИО, почта, подразделение, пароль) меняет **только администратор** в разделе «Администрирование» → «Управление пользователями». Кнопка «Профиль» в шапке — только просмотр.
